...
namespace GE {
	...
	void ModelRenderer::draw(Camera* cam, Model* model, Terrain*terrain, Skydome*skydome, std::string name)
	{
		...
		if (name == "skydome")
		{
		 transformationMat = glm::translate(transformationMat, glm::vec3(cam->getPosX(), cam->getPosY(), cam->getPosZ()));
		}
		else
		{
	...
	void ModelRenderer::drawskydome(GE::Skydome* skydome)
	{
	...
	void ModelRenderer::drawterrain(GE::Terrain* terrain)
	{
	...
	void ModelRenderer::drawmodel(GE::Model* model)
	{
